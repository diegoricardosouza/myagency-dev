...
const schema = z.object({
  description: z.string()
    .min(1, 'Informações Técnicas é obrigatório'),
});

type FormData = z.infer<typeof schema>

export function useTechnicalInformationController() {
  ...
  useEffect(() => {
    if (techData) {
      setValue('description', techData.data.technical_information)
    }
  }, [setValue, techData]);
  ...
  const handleSubmit = hookFormSubmit(async (data) => {
    try {
      if (id) {
        await mutateAsync({
          technical_information: data.description,
          id
        });
      }
